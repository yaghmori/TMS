        ...
        private async void SaveChangesSubmit(EditContext context)
        {
            ...
            if (string.IsNullOrWhiteSpace(TenantId)) //New
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        dgResult = true;
                        _snackbar.Add(_messageLocalizer["ClientSuccessfullyCreated"].Value, Severity.Success);
                    }
                    else
                    {
                        dgResult = false;
                        await _TenantDataService.DeleteTenantByIdAsync(tenantId);
                        _snackbar.Add(_messageLocalizer["CreatingClientFailed"].Value, Severity.Error);
            ...
            else //Edit
            {
                ...
                {
                    dgResult = true;
                    _snackbar.Add(_messageLocalizer["ClientSuccessfullyUpdated"].Value, Severity.Success);

                    //SignalR
                    var usersResponse = await _TenantDataService.GetUsersByTenantId(TenantId);
                ...
                {
                    dgResult = false;
                    _snackbar.Add(_messageLocalizer["UpdatingClientFailed"].Value, Severity.Error);
